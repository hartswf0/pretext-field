...
// ═══════════════════════════════════════════════════════════
// THEME — read colors from CSS custom properties at boot.
// This lets the same engine serve both the CRT (amber/gold)
// and Clean (blue/gray) HTML shells.
// ═══════════════════════════════════════════════════════════
const _cs = getComputedStyle(document.documentElement)
const _cv = (v: string, fb: string) => _cs.getPropertyValue(v).trim() || fb
const THEME = {
  accent:      _cv('--accent',       '#00ccaa'),
  gold:        _cv('--gold',         '#d4af37'),
  amber:       _cv('--amber',        '#ff8c00'),
  rust:        _cv('--rust',         '#8b4513'),
  highlight:   _cv('--highlight',    '#ff0'),
  bgDark:      _cv('--bg-dark',      '#0a0a0a'),
  bgCard:      _cv('--bg-card',      '#1a1a1a'),
...
function editorOpen(zoneKey: string): void {
  ...
  for (const k of allKeys) {
    ...
    btn.style.cssText = `flex-shrink:0; font-size:11px; font-family:Courier New; font-weight:900;
      padding:6px 12px; border:none; cursor:pointer; letter-spacing:0.5px; white-space:nowrap;
      background:${isActive ? THEME.accent : THEME.bgCard}; color:${isActive ? '#fff' : THEME.textPrimary};
      border-bottom:${isActive ? '2px solid ' + THEME.accent : '2px solid transparent'};`
...
const GRID_COLORS = [THEME.gold, '#4682b4', '#6366f1', '#06b6d4', '#10b981', '#8b5cf6', '#ec4899', '#f59e0b']
for (let i = 1; i <= 16; i++) {
...
function renderLidar(): void {
  ...
  lidarCtx.font = 'bold 14px Courier New'
  lidarCtx.fillStyle = '#fff'
  lidarCtx.fillText(`${bearingLabel} ${Math.round(bearing)}°`, cw / 2, 10)

  // Zone label below compass
  if (currentZoneLabel) {
    lidarCtx.font = 'bold 11px Courier New'
    lidarCtx.fillStyle = ZONES[currentZoneKey!]?.color || '#888'
    lidarCtx.fillText(`${currentZoneLabel} · ${Math.round(nearestPanelDist)}m`, cw / 2, 26)
  }

  // Zone teleport shortcuts — subtle key hints
  const zkArr = Object.keys(MOUNTAIN_LORE)
  if (zkArr.length > 1) {
    lidarCtx.font = 'bold 10px Courier New'
    lidarCtx.globalAlpha = 0.6
    const shortcutText = zkArr.map((k, i) => `${i + 1}:${(ZONE_LABELS[k] || k).substring(0, 8)}`).join('  ')
    lidarCtx.fillStyle = '#ccc'
    lidarCtx.fillText(shortcutText, cw / 2, 40)
  }
  ...
  for (const face of faces) {
    ...
    if (isCenter && faceW > 60) {
      ...
      lidarCtx.globalAlpha = 0.7 + 0.2 * Math.sin(time * 0.3)
      if (editorActive && editorZone === face.loreKey) {
        lidarCtx.fillStyle = THEME.accent
        const blockCount = (BLOCK_STORE[face.loreKey] || []).length
        lidarCtx.fillText(`✎ EDITING · ${blockCount} BLOCKS`, face.startX + padding, textBot - 16)
      } else {
        ...
        lidarCtx.fillStyle = THEME.accent
        lidarCtx.fillText(`[E] EDIT · @ LINK · ${blockCount} ¶${overflowText}`, face.startX + padding, textBot - 16)
      }
  ...
  if (cw > 600) {
    ...
    lidarCtx.textAlign = 'center'
    lidarCtx.textBaseline = 'bottom'
    lidarCtx.fillStyle = '#999'
    const skyLabel = skyModeLock === 'auto' ? '' : skyModeLock === 'light' ? ' · ☀ LIGHT' : ' · ☽ DARK'
    lidarCtx.fillText(`WASD · EDIT [E] · SEARCH [⌘F] · SKY [L] · TELEPORT [1-5]${skyLabel}`, cw / 2, ch - 14)
  }
...
;(document.getElementById('editor-bar-textarea') as HTMLTextAreaElement).addEventListener('input', (e) => {
  ...
  for (const c of candidates.slice(0, 12)) {
    ...
    const typeColor = c.type === 'panel' ? '#0ca' : THEME.gold
...
function buildOneShotImport(): void {
  ...
  fullDiv.innerHTML = `
    <div class="sh-label" style="color:${THEME.highlight};">⚡ ONE-SHOT IMPORT · LLM CONVERTER</div>
    ...
    <div style="display:flex; gap:8px; margin-bottom:8px;">
      <button class="sh-btn" id="btn-copy-prompt" style="font-size:9px; padding:3px 10px; border-color:${THEME.highlight}; color:${THEME.highlight};">⎘ COPY SYSTEM PROMPT</button>
      <span id="prompt-status" style="font-size:9px; color:#555; line-height:24px;">Ready</span>
    </div>
    <textarea id="full-import-text" style="width:100%; height:140px; background:${THEME.bgDark}; color:${THEME.textPrimary}; border:1px solid ${THEME.border}; padding:8px; font-family:Courier New; font-size:10px; resize:vertical;" placeholder='Paste the full JSON from the LLM here: {"zones":[...],"sceneGraph":[...]}'></textarea>
    <button class="sh-btn primary" id="btn-full-import" style="margin-top:6px; background:${THEME.highlight}; border-color:${THEME.highlight}; color:#000;">⚡ IMPORT ALL (ZONES + PORTALS)</button>
  `
  loreBody.appendChild(fullDiv)

  // System prompt text (embedded) — matches the 16-panel architecture
  const panelKeys = Object.keys(ZONE_LABELS)
  const panelList = panelKeys.slice(0, 8).map(k => `"${k}"`).join(', ')
  const SYSTEM_PROMPT = `You are a Worldtext Architect. The user will give you a text — any text: a poem, a recipe, a legal document, a novel chapter, field notes, song lyrics, a Wikipedia article. Your job is to convert it into a structured JSON payload for the Golden Egg Worldtext Engine.

The engine has up to 16 PANELS. Each panel is a wall that displays text. Words that appear in multiple panels become clickable HYPERPORTALS that teleport the reader between walls.

OUTPUT FORMAT — return ONLY valid JSON, no markdown fences, no explanation:

{
  "zones": [
    { "key": "panel_1", "label": "YOUR PANEL 1 NAME", "text": ["Line 1.", "Line 2.", "..."] },
    { "key": "panel_2", "label": "YOUR PANEL 2 NAME", "text": ["..."] },
    { "key": "panel_3", "label": "YOUR PANEL 3 NAME", "text": ["..."] },
    { "key": "panel_4", "label": "YOUR PANEL 4 NAME", "text": ["..."] },
    { "key": "panel_5", "label": "YOUR PANEL 5 NAME", "text": ["..."] }
  ],
  "sceneGraph": [
    { "word": "keyword", "zone": "panel_1", "connections": ["other_keyword"], "clue": "Short description." }
  ]
}

RULES FOR PANELS:
1. Panel keys MUST use the format: panel_1, panel_2, panel_3, ... panel_16 (internal IDs).
2. Give each panel a descriptive "label" fitting the source text.
3. Split the source text into 3-8 thematic sections. Each becomes a panel. You do not need to use all 16.
...
2. Each keyword gets a "zone" — its HOME panel where it is most important (e.g. "panel_1").
3. "connections" lists related keywords in OTHER panels — this creates the navigation web.
4. "clue" is 1-2 sentences: what is this word, why does it matter, what does it connect?
5. CRITICAL: The best portals are words whose HOME panel is different from the panel text they appear in. For example, if "gold" has zone "panel_2" but also appears in panel_4 text ("golden clusters"), then "gold" becomes a glowing portal on panel_4 walls that teleports to panel_2. ACTIVELY PLANT keywords in other panels' text to create these cross-links.
6. Words that only appear in their own panel still work — they show as local references with a dotted underline and show clue text when clicked. But cross-panel words are the TELEPORTATION hyperlinks.
7. Aim for at least 3-5 cross-panel keywords per panel. Repeat important words in multiple panels' text arrays to create the portal web.
...
FORMATTING RULES FOR TEXT LINES:
1. Use "# Title" as the FIRST line of each panel's text array — this becomes a large heading on the wall.
2. Use "## Subtitle" for section subheadings within a panel.
3. Use "> quote text" for block quotes — they render indented with a bar.
4. Use "- item" for bullet lists.
5. Use "---" alone on a line for a horizontal rule separator.
6. Keep each line 40-120 characters for optimal wall readability.
7. URLs (https://...) in text lines auto-highlight as clickable links on the wall.
8. Repeat important sceneGraph keywords naturally in the text so they become glowing portals.`

  // Copy prompt handler
  document.getElementById('btn-copy-prompt')!.addEventListener('click', () => {
    navigator.clipboard.writeText(SYSTEM_PROMPT).then(() => {
      const s = document.getElementById('prompt-status')
      if (s) { s.innerText = 'COPIED ✓ — paste into any LLM'; s.style.color = THEME.highlight
  ...
  llmDiv.style.cssText = `margin-top:16px; border-top:1px solid ${THEME.border}; padding-top:12px;`
  const isConnected = llmKey.length > 0
  const statusColor = isConnected ? THEME.accent : THEME.textMuted
  const statusText = isConnected ? `✓ Connected: ${llmModel}` : 'Paste API key to connect'
  llmDiv.innerHTML = `
    <div style="color:${THEME.accent}; font-size:10px; letter-spacing:2px; margin-bottom:8px;">🤖 LLM ENGINE · <span style="color:${statusColor}; font-size:9px;">${statusText}</span></div>
    <div style="font-size:9px; color:${THEME.textMuted}; margin-bottom:8px;">Paste your API key below. Open any panel (Press E) and start a new line with // your prompt and press Enter to chat.</div>
    <div style="display:flex; gap:6px; margin-bottom:6px;">
      <input type="password" id="llm-key-input" placeholder="sk-... (OpenAI API Key)" 
        value="${llmKey}" style="flex:1; background:${THEME.bgDark}; color:${THEME.textPrimary}; border:1px solid ${isConnected ? THEME.accent : THEME.border}; padding:6px 10px; font-size:11px; font-family:Courier New;">
      <button class="sh-btn" id="btn-llm-connect" style="font-size:10px; padding:4px 16px; border-color:${THEME.accent}; color:${THEME.accent};">${isConnected ? '✓ CONNECTED' : 'CONNECT'}</button>
    </div>
    <details style="margin-top:6px;">
      <summary style="cursor:pointer; color:${THEME.textMuted}; font-size:9px;">Advanced: change model or API endpoint</summary>
      <div style="display:flex; gap:6px; margin-top:6px;">
        <input type="text" id="llm-url-input" placeholder="API URL" 
          value="${llmUrl}" style="flex:2; min-width:180px; background:${THEME.bgDark}; color:${THEME.textMuted}; border:1px solid ${THEME.border}; padding:4px 8px; font-size:9px;">
        <input type="text" id="llm-model-input" placeholder="Model" 
          value="${llmModel}" style="flex:1; min-width:60px; background:${THEME.bgDark}; color:${THEME.textMuted}; border:1px solid ${THEME.border}; padding:4px 8px; font-size:9px;">
      </div>
    </details>
  `
...
function populateLoreSheet(): void {
  ...
  for (const [key, texts] of Object.entries(MOUNTAIN_LORE)) {
    ...
    div.innerHTML =
      ...
      texts.slice(0, 20).map(t => `<div style="font-size:11px; color:${THEME.textPrimary}; padding:3px 0; border-bottom:1px solid ${THEME.bgCard};">${t}</div>`).join('') +
      (texts.length > 20 ? `<div style="font-size:10px; color:${THEME.textMuted}; padding:4px;">... +${texts.length - 20} more</div>` : '')
  ...
  uploadDiv.innerHTML = `
    <div class="sh-label" style="color:${THEME.accent};">UPLOAD TEXT</div>
    <div style="font-size:10px; color:${THEME.textMuted}; margin-bottom:6px;">Paste text below. Each line becomes a wall entry. Use REPLACE to clear old text first.</div>
    <div style="display:flex; gap:8px; margin-bottom:6px; align-items:center;">
      <select id="upload-zone" style="background:${THEME.bgCard}; color:${THEME.textPrimary}; border:1px solid ${THEME.border}; padding:4px 8px; font-size:11px; flex:1;">
        ${zoneOptions}
      </select>
      <input type="text" id="rename-zone" placeholder="Rename zone..." style="background:${THEME.bgCard}; color:${THEME.textPrimary}; border:1px solid ${THEME.border}; padding:4px 8px; font-size:10px; width:120px;">
      <button class="sh-btn" id="btn-rename-zone" style="font-size:9px; padding:2px 8px; min-height:auto;">RENAME</button>
    </div>
    <textarea id="upload-text" style="width:100%; height:120px; background:${THEME.bgDark}; color:${THEME.textPrimary}; border:1px solid ${THEME.border}; padding:8px; font-family:Courier New; font-size:11px; resize:vertical;" placeholder="Paste any text here: Homer, recipes, law, field notes..."></textarea>
  ...
  graphDiv.style.borderTop = `1px solid ${THEME.border}`
  graphDiv.style.paddingTop = '16px'
  graphDiv.innerHTML = `
    <div class="sh-label" style="color:${THEME.amber};">⬡ SCENE GRAPH · HYPERPORTAL LINKAGES</div>
    <div style="font-size:10px; color:${THEME.textMuted}; margin-bottom:8px; line-height:1.4;">
      Export the current clue graph or upload new nodes. Each node becomes a clickable hyperportal on the walls.<br>
      Format: <code style="color:${THEME.accent}; font-size:9px;">[{"word":"lamp","zone":"panel_1","connections":["beam","flashlight"],"clue":"Oil lamp. 1842 vintage."}]</code>
    </div>
    <div style="display:flex; gap:8px; margin-bottom:8px;">
      ...
      <span id="graph-status" style="font-size:9px; color:${THEME.textMuted}; line-height:24px;">${Object.keys(CLUE_GRAPH).length} nodes</span>
    </div>
    <textarea id="upload-graph" style="width:100%; height:100px; background:${THEME.bgDark}; color:${THEME.textPrimary}; border:1px solid ${THEME.border}; padding:8px; font-family:Courier New; font-size:10px; resize:vertical;" placeholder='[{"word":"lamp","zone":"panel_1","connections":["beam"],"clue":"Oil lamp."}]'></textarea>
...
function populateCluesSheet(): void {
  ...
  header.innerHTML = `<div style="color:${THEME.gold}; font-size:11px; letter-spacing:2px; margin-bottom:12px;">PANEL INDEX · ${panelCount} PANELS · ${totalWords} WORDS</div>`
...
function showHelpOverlay() {
  ...
  ob.innerHTML = `<div style="max-width:500px; padding:32px; border:2px solid ${THEME.accent}; background:${THEME.bgDark}; text-align:center; overflow-y:auto; max-height:90vh;">
    ...
    <div style="font-size:11px; color:${THEME.textMuted}; line-height:1.8; text-align:left; margin-bottom:20px;">
      ...
      <div><span style="color:${THEME.accent};">MOUSE</span> — Look around (drag)</div>
      <div><span style="color:${THEME.accent};">CLICK WALL</span> or <span style="color:${THEME.accent};">[E]</span> — Edit the panel you face</div>
      <div><span style="color:${THEME.accent};">@</span> in editor — Autocomplete panel links (replaces [[word]])</div>
      <div><span style="color:${THEME.accent};">⌘F / Ctrl+F</span> — Search across all panels</div>
      <div><span style="color:${THEME.accent};">DROP .txt/.md</span> — Import a file onto a wall</div>
      <div><span style="color:${THEME.accent};">↶ UNDO</span> — Undo wall edits (20 levels per panel)</div>
      <div><span style="color:${THEME.accent};">ESC</span> — Close editor / exit</div>
      <div style="margin-top:8px; color:${THEME.textMuted};">Sheet D (tab bar) → BUILD NEW MONOLITH to add panels</div>

      <div style="color:${THEME.gold}; font-weight:bold; margin-bottom:8px; margin-top:20px;">HOW TO DEPLOY THE LLM:</div>
      <div><span style="color:${THEME.gold};">1.</span> Click the <b>A</b> tab (LORE MATERIAL) in the bottom right corner.</div>
...
if ('ontouchstart' in window || navigator.maxTouchPoints > 0) {
  ...
  for (const [label, icon, action] of mobileActions) {
    ...
    btn.style.cssText = `width:48px; height:48px; border-radius:50%; background:rgba(0,0,0,0.6);
      border:1px solid rgba(255,255,255,0.2); color:${THEME.gold}; font-family:Courier New;
    ...
    btn.innerHTML = `<span style="font-size:16px;">${icon}</span><span style="font-size:9px; color:${THEME.textMuted};">${label}</span>`

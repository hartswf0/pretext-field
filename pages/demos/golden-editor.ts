...
const STORAGE_KEY = 'golden-editor-lore'
...
function exportMarkdown(): void {
  ...
  let md = `# Golden Editor — Worldtext Export\n\n`
  ...
  a.href = url
  a.download = `golden-editor-${Date.now()}.md`
  a.click()
...
const MOUNTAIN_LORE: Record<string, string[]> = {
  shack: [
    "GOLDEN EDITOR · PRETEXT WORLDTEXT EDITOR. This is Zone 1.",
    "Click any wall in LiDAR mode to open the inline editor.",
    "Type directly — your text appears on the walls in real time.",
    "Each zone is an editable room. Five zones, five documents.",
    "Your edits auto-save to localStorage. Reload and they persist.",
    "The walls are the document. The document is the world.",
    "Use Sheet A to bulk-import text. Use the walls to refine it.",
    "Every surface is a pretext-enabled text editor.",
  ],
  creek: [
    "ZONE 2 · EDITOR. Click this wall to start writing.",
    "Cross-zone vocabulary still creates hyperportals.",
    "Write something mentioning a Zone 1 concept — it becomes a link.",
    "The editor supports full paragraph editing with live re-layout.",
    "Press E in LiDAR mode to edit the wall you're facing.",
  ],
  ridge: [
    "ZONE 3 · EDITOR. Your third writing surface.",
    "Use the Scene Graph in Sheet A to define clickable keywords.",
    "Each keyword becomes a glowing hyperportal on the walls.",
    "Walk between zones. Edit each one. Build a connected document.",
  ],
  cave: [
    "ZONE 4 · EDITOR. Walk in. Read the walls. Edit them.",
    "WASD to move. E to edit. SPACE to scan. Click portals to navigate.",
    "The pretext engine re-wraps your text to fit each wall face.",
  ],
  puddle: [
    "ZONE 5 · EDITOR. The outermost writing surface.",
    "Connect an LLM via URL params to generate text into any zone.",
    "Or just type. The walls accept whatever you give them.",
  ],
}
...
document.getElementById('btn-export-png')!.addEventListener('click', () => {
  ...
  const link = document.createElement('a')
  link.download = `golden-editor-${Date.now()}.png`; link.href = c.toDataURL('image/png'); link.click()
})
document.getElementById('btn-export-lore')!.addEventListener('click', () => {
  ...
  const link = document.createElement('a')
  link.download = `golden-editor-lore-${Date.now()}.json`; link.href = URL.createObjectURL(blob); link.click()
})

...
const MOUNTAIN_LORE: Record<string, string[]> = {
  shack: [
    "GOLDEN SHIELD · WORLDTEXT ENGINE. This is Zone 1.",
    "Open Sheet A (tab at bottom) to upload your own text.",
    "Each zone becomes a room whose walls display your prose.",
    "Paste your text, select a zone, press INJECT INTO WALLS.",
    "Words from other zones glow as clickable hyperportals.",
    "Click a glowing word to teleport to its home zone.",
    "The Shield of Achilles. Biscuit recipes. The US Constitution. Any text becomes a navigable world.",
    "Upload a Scene Graph (at the bottom of Sheet A) to define which words are portals.",
    "Format: [{\"word\":\"Pleiades\",\"zone\":\"cosmos\",\"connections\":[\"Orion\"],\"clue\":\"Seven sisters.\"}]",
  ],
  creek: [
    "ZONE 2. Upload your own text here via Sheet A.",
    "Cross-zone vocabulary creates the navigation web.",
    "If you mention a Zone 1 word here, it becomes a portal back to Zone 1.",
    "If you mention a Zone 3 word here, it becomes a portal forward to Zone 3.",
    "The denser the cross-references, the richer the hypertext navigation.",
    "Press G in LiDAR mode to generate new text via LLM.",
  ],
  ridge: [
    "ZONE 3. Upload your own text here via Sheet A.",
    "Use the Scene Graph section in Sheet A to define portals.",
    "Each keyword needs: word, zone, connections, clue.",
    "The minimap in LiDAR shows the investigation web.",
    "Zone clusters appear as colored dots. Connections as thread lines.",
  ],
  cave: [
    "ZONE 4. Upload your own text here via Sheet A.",
    "Enter LiDAR mode to walk through your text world.",
    "WASD to move. SPACE to scan. Click glowing words to navigate.",
    "The scanner output shows clue text when you click a portal.",
  ],
  puddle: [
    "ZONE 5. Upload your own text here via Sheet A.",
    "To connect an LLM: add ?llm_url=http://localhost:11434/api/generate&llm_model=llama3 to the URL.",
    "The LLM will generate new zone-specific text on demand.",
    "All five zones can hold unlimited text. The walls expand as you add more.",
  ],
}
...
// ═══ ZONE SYSTEM — dynamically labeled ═══
const ZONE_LABELS: Record<string, string> = {
  shack: 'ZONE 1', creek: 'ZONE 2', ridge: 'ZONE 3', cave: 'ZONE 4', puddle: 'ZONE 5',
}

const ZONES: Record<string, { name: string; color: string; minR: number; maxR: number; flavor: string }> = {
  SHACK:  { name: 'ZONE 1', color: '#daa520', minR: 0,  maxR: 4,  flavor: 'ZONE 1' },
  CREEK:  { name: 'ZONE 2', color: '#4682b4', minR: 4,  maxR: 8,  flavor: 'ZONE 2' },
  RIDGE:  { name: 'ZONE 3', color: '#cd853f', minR: 8,  maxR: 13, flavor: 'ZONE 3' },
  PUDDLE: { name: 'ZONE 4', color: '#5f9ea0', minR: 13, maxR: 16, flavor: 'ZONE 4' },
  CAVE:   { name: 'ZONE 5', color: '#9370db', minR: 16, maxR: 99, flavor: 'ZONE 5' },
}
...
function generateCircularMap(zoneKeys: string[]): void {
  const N = zoneKeys.length
  if (N === 0) return

  // ═══ CONCENTRIC RING ARCHITECTURE ═══
  // Each zone = one wide concentric hall.
  // Few radial walls create real rooms, not a maze of cells.
  // Wide doorways between rings for natural flow.

  const zoneCounts: number[] = zoneKeys.map(k => MOUNTAIN_LORE[k]?.length || 1)

  // Ring geometry — wide enough for real halls
  const ringWidth = 6    // each ring: 1 wall + 4 floor + 1 wall
  const hallWidth = 4    // walkable floor cells in each ring
  const coreRadius = 5   // spacious center plaza
  const outerRadius = coreRadius + N * ringWidth
  const sz = outerRadius * 2 + 5
  const szOdd = sz % 2 === 0 ? sz + 1 : sz
  const center = Math.floor(szOdd / 2)

  // Initialize grid — all solid
  const grid: string[][] = Array.from({ length: szOdd }, () =>
    Array.from({ length: szOdd }, () => '#')
  )

  // Carve the center core (open plaza — generous space)
  for (let y = 0; y < szOdd; y++) {
    for (let x = 0; x < szOdd; x++) {
  ...
  // ═══ BUILD EACH RING ═══
  for (let z = 0; z < N; z++) {
    const innerWallR = coreRadius + z * ringWidth           // inner text wall
    const outerWallR = coreRadius + (z + 1) * ringWidth - 1 // outer text wall (shared with next ring's inner)

    // Carve the wide hall between the walls
    for (let y = 0; y < szOdd; y++) {
      for (let x = 0; x < szOdd; x++) {
        const dist = Math.sqrt((x - center) ** 2 + (y - center) ** 2)
        // Floor: everything between inner wall and outer wall
        if (dist > innerWallR + 0.5 && dist < outerWallR - 0.5) {
          grid[y]![x] = '.'
        }
      }
    }

    // Place zone-marked text walls on the INNER wall
    for (let y = 0; y < szOdd; y++) {
      for (let x = 0; x < szOdd; x++) {
        const dist = Math.sqrt((x - center) ** 2 + (y - center) ** 2)
        if (dist >= innerWallR - 0.5 && dist < innerWallR + 0.5) {
          grid[y]![x] = String(z + 1)
        }
      }
    }

    // Place zone-marked text walls on the OUTER wall
    for (let y = 0; y < szOdd; y++) {
      for (let x = 0; x < szOdd; x++) {
        const dist = Math.sqrt((x - center) ** 2 + (y - center) ** 2)
        if (dist >= outerWallR - 0.5 && dist < outerWallR + 0.5) {
          grid[y]![x] = String(z + 1)
        }
      }
    }
  ...
  // ═══ RADIAL WALLS ═══
  // Placed in diagonal octants to avoid blocking the 4 main corridors.
  // Creates large rooms between the mall aisles.

  for (let z = 0; z < N; z++) {
    const numWalls = Math.min(4, Math.max(2, Math.ceil(zoneCounts[z]! / 3)))
    const innerWallR = coreRadius + z * ringWidth
    const outerWallR = coreRadius + (z + 1) * ringWidth - 1

    for (let w = 0; w < numWalls; w++) {
      // Place walls in diagonal quadrants (π/4, 3π/4, 5π/4, 7π/4 base)
      // offset per ring so they don't all align
      const baseAngle = (Math.PI / 4) + (w / numWalls) * Math.PI * 2 + (z * 0.3)
      const angle = baseAngle

      // Draw radial wall from inner to outer
      for (let r = innerWallR + 1; r < outerWallR; r += 0.3) {
        const wx = Math.round(center + r * Math.cos(angle))
        const wy = Math.round(center + r * Math.sin(angle))
        if (wx > 0 && wx < szOdd - 1 && wy > 0 && wy < szOdd - 1) {
          if (grid[wy]![wx] === '.') {
            grid[wy]![wx] = String(z + 1)
          }
        }
      }
    }
  ...
  // ═══ MAIN CORRIDORS — 4 cardinal aisles through ALL rings ═══
  // Like a circular mall: N/S/E/W aisles always open, 3+ cells wide.
  // You can always walk straight from center to the outermost ring.
  ...
  const totalRadius = coreRadius + N * ringWidth
  for (const cardAngle of cardinalAngles) {
    // Cut a wide corridor from core to outer rim
    for (let r = 0; r < totalRadius + 1; r += 0.3) {
      for (let spread = -1.2; spread <= 1.2; spread += 0.3) {
        // perpendicular offset for width
        const cx = Math.round(center + r * Math.cos(cardAngle) + spread * Math.cos(cardAngle + Math.PI / 2))
        const cy = Math.round(center + r * Math.sin(cardAngle) + spread * Math.sin(cardAngle + Math.PI / 2))
        if (cx > 0 && cx < szOdd - 1 && cy > 0 && cy < szOdd - 1) {
          grid[cy]![cx] = '.'
        }
      }
    }
  ...
  // ═══ SECONDARY DOORWAYS ═══
  // 2 extra doors per ring at diagonal angles for variety

  for (let z = 0; z < N; z++) {
    const innerWallR = coreRadius + z * ringWidth
    const diagAngles = [
      Math.PI / 4 + (z * 0.2),   // NE-ish, offset per ring
      -3 * Math.PI / 4 + (z * 0.2), // SW-ish, offset per ring
    ]
    for (const dAngle of diagAngles) {
      for (let offset = -0.25; offset <= 0.25; offset += 0.08) {
        const a = dAngle + offset
        const dx = Math.round(center + innerWallR * Math.cos(a))
        const dy = Math.round(center + innerWallR * Math.sin(a))
        if (dx > 0 && dx < szOdd - 1 && dy > 0 && dy < szOdd - 1) {
          grid[dy]![dx] = '.'
        }
      }
    }
  ...
  // ═══ ZONE POSITIONS ═══
  // Each zone target is in the middle of its ring hall
  const zoneArcs: Record<string, { x: number; y: number }> = {}
  for (let z = 0; z < N; z++) {
    const hallR = coreRadius + z * ringWidth + ringWidth / 2
    const angle = -Math.PI / 2 + (z * 0.4)
    zoneArcs[zoneKeys[z]!] = {
      x: Math.round(center + hallR * Math.cos(angle)),
      y: Math.round(center + hallR * Math.sin(angle)),
    }
  }
  ...
  // Rebuild ZONE_POSITIONS
  for (const key of Object.keys(ZONE_POSITIONS)) {
    delete ZONE_POSITIONS[key]
  }
  for (const [key, pos] of Object.entries(zoneArcs)) {
    ZONE_POSITIONS[key] = pos
  }

  // Reset player to center — the innermost fold of the shield
  player.x = center + 0.5
  player.y = center + 0.5
...
// Initial map generation with default zones
...
document.getElementById('btn-export-png')!.addEventListener('click', () => {
  ...
  const link = document.createElement('a')
  link.download = `golden-shield-${Date.now()}.png`; link.href = c.toDataURL('image/png'); link.click()
})
document.getElementById('btn-export-lore')!.addEventListener('click', () => {
  ...
  const link = document.createElement('a')
  link.download = 'golden-shield-lore.txt'; link.href = URL.createObjectURL(blob); link.click()
})
